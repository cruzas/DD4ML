...
CRITERION_MAP = {
    ...
    "weighted_cross_entropy": ("", lambda ds: nn.CrossEntropyLoss(weight=ds.compute_class_weights())),
    "mse": ("", lambda ds=None: nn.MSELoss()),
    "cross_entropy_transformers": ("", lambda ds=None: cross_entropy_transformers)  # Assumes defined elsewhere.
}
...
def get_config(dataset_name: str, model_name: str, optimizer: str):
    from dd4ml.trainer import \
        Trainer  # Assumes Trainer and CfgNode are defined in dd4ml.trainer.
...
def get_config_model_and_trainer(args, wandb_config):
    ...
    if getattr(all_config.trainer, "use_pmw", False) and hasattr(all_config.model.model_class, "as_model_dict"):
    ...
    criterion = criterion_factory.create(criterion_key, dataset if "weighted" in criterion_key else None)
    ...
    if optimizer_name in OPTIMIZER_MAP:
        ...
        for attr in ["subdomain_optimizer", "subdomain_optimizer_args", "global_optimizer", "global_optimizer_args"]:
    ...
    trainer = Trainer(all_config.trainer, model, optimizer_obj, criterion, dataset, test_dataset)
