...
class TrustRegion(Optimizer):
    ...
    def __init__(
        ...
        self.nu = min(nu, nu_1)
        self.max_iter = max_iter  # Used here as the maximum memory size for curvature pairs
        ...
        self.state['u_history'] = []  # List to store past curvature differences (u = y - B0 * s)
    ...
    def _solve_tr_subproblem(self, g, delta):
        ...
        for _ in range(min(n, 50)):
            ...
            if pBp <= 1e-12:  # negative curvature detected
                ...
                disc = B_val**2 - 4*A*C
                disc = max(disc, 0.0)
                alpha = (-B_val + math.sqrt(disc)) / (2*A) if B_val < 0 else (-B_val - math.sqrt(disc)) / (2*A)
                alpha = max(alpha, 0.0)
    ...
    def step(self, closure, old_loss=None, grad=None):
        ...
        if grad is None:
            if not self.model_has_grad:
                grad = torch.cat([p.grad.detach().view(-1) for p in self.param_list if p.grad is not None])
            else:
        ...
        if not self.state['initialized']:
            ...
            self.state['prev_params'] = torch.cat(params_flat) if params_flat else torch.tensor([], device=grad.device)
        ...
        params_flat_current = torch.cat(params_flat_current) if params_flat_current else torch.tensor([], device=grad.device)
        ...
        actual_reduction = old_loss - new_loss_val if (old_loss is not None and new_loss_val is not None) else None
        rho = actual_reduction / (predicted_reduction + 1e-12) if (actual_reduction is not None and predicted_reduction != 0) else float('inf')

        # Adjust the trust-region radius (lr) and decide acceptance.
        if (actual_reduction is not None and actual_reduction > 0) and (rho >= self.nu_1):
            # Accept step.
            ...
            for p in self.param_list:
                if p.numel() == 0:
                    continue
                numel = p.numel()
                p.data.copy_(params_flat_current[offset:offset+numel].view_as(p.data))
                offset += numel
        ...
        self.state['prev_params'] = torch.cat(params_flat_new) if params_flat_new else torch.tensor([], device=grad.device)
        ...
        if abs(denom.item()) > 1e-8 * torch.norm(u_vec).item() * torch.norm(s_vec).item():
