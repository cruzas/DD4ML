...
class BaseModel(nn.Module):
    ...
    def distributed_model_rank_structure(self, subdomains, replicas_per_subdomain, stages_amount, gpus_per_sharded_layer, node_rank_dict):
                ...
                for i in range(stages_amount):
                    ...
                    for j in range(gpus_per_sharded_layer):
                        ...
                        if f'stage{i}_shard{j}' not in layer_copies:
                            ...
                            layer_copies[f'stage{i}_shard{j}'].append(
                                rank_list[c])
                        if i == stages_amount - 1 and j == 0:
                            subdomain_final_stages_main_rank[sd].append(
                                rank_list[c])
                        if i == stages_amount - 1 and j == 0:
        ...
        self.last_layers_main_shard = layer_copies['stage' +
                                                   str(stages_amount-1)+'_shard0']
        ...
        self.subdomain_ranks_group = dist.new_group(
            ranks=self.subdomain_rank_structure(flatten=True), use_local_synchronization=True)
        ...
        for sd in range(subdomains):
            if self.rank in subdomain_final_stages_main_rank[sd]:
                ...
                self.subdomain_final_stages_main_rank_group = dist.new_group(
                    ranks=self.subdomain_final_stages_main_rank, use_local_synchronization=True)
                break
        self.replicas_in_subdomain_final_stages_main_rank = [
            r for r in self.subdomain_final_stages_main_rank if r in self.subdomain_ranks]
        self.replicas_in_subdomain_final_stages_main_rank_group = dist.new_group(
            ranks=self.replicas_in_subdomain_final_stages_main_rank, use_local_synchronization=True)
