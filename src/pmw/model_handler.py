...
def resolve_backward_dependencies(model):
    start = 'finish'
    max_distances = {layer: max_path_length(
        model, layer) for layer in model.keys()}
    max_distances = list(dict(
        sorted(max_distances.items(), key=lambda item: item[1], reverse=False)).keys())
    done = []
    while True:
        dst_layers = model[start]['rcv']['src']
        max_path_lengths = {layer: max_path_length(
            model, layer) for layer in dst_layers}
...
def resolve_layer_dependencies(model):
    ...
    dependencies = {layer: set(info['rcv']['src'])
                    for layer, info in model.items()}
    dependents = {layer: set(info['dst']['to'])
                  for layer, info in model.items()}
    incoming_edges_count = {layer: len(srcs)
                            for layer, srcs in dependencies.items()}

    # Start with nodes with no incoming edges (no dependencies)
    to_process = [layer for layer,
                  count in incoming_edges_count.items() if count == 0]
    ordered_layers = []

    # Perform topological sort
    while to_process:
        # Remove the first element, similar to popleft()
...
class ModelHandler():
    def __init__(self, net_dict, num_subdomains, num_replicas_per_subdomain, available_ranks=None):
        ...
        self.available_ranks = sorted(available_ranks) if available_ranks is not None else list(
    ...
    def get_stage_ranks(self, stage_name, mode):
        assert mode in [
            'local', 'global', 'replica'], f"Invalid mode '{mode}'. Must be either 'global', 'local', or 'replica'."
        assert stage_name in [
            'first', 'last'], f"Invalid stage '{stage_name}'. Must be either 'first' or 'last'."
        ...
        def _go_through_replicas(sd, stage, stage_ranks):
            for rep in range(self.num_replicas_per_subdomain):
                ...
                assert len(
                    ranks) == 1, f"Tensor sharding not implemented yet. Expected 1 rank, got {len(ranks)}."
    ...
    def get_layers_copy_group(self, mode='global'):
        if mode not in ['local', 'global']:
            raise ValueError(
                f"Invalid mode '{mode}'. Must be either 'global' or 'local'.")
            ...
            for sd in range(self.num_subdomains):
                for rep in range(self.num_replicas_per_subdomain):
                    for s in range(self.num_stages):
                        ...
                        num_shards = self.net_dict[layer_names[0]
                                                   ]["num_layer_shards"]
    ...
    def create_distributed_model_rank_structure(self):
        if len(self.available_ranks) < self.num_subdomains*self.num_replicas_per_subdomain*self.num_ranks_per_model:
            ...
            print(
                f"Warning: Number of available ranks ({len(self.available_ranks)}) is more than the required number of ranks ({self.num_subdomains*self.num_replicas_per_subdomain*self.num_ranks_per_model})... some will be idle.")
        ...
        subdomain_ranks = [self.available_ranks[i *
                                                n:(i+1)*n] for i in range(0, self.num_subdomains)]
        ...
        for sd in range(self.num_subdomains):
            ...
            for rep in range(self.num_replicas_per_subdomain):
                ...
                rep_ranks = subdomain_ranks[sd][rep *
                                                self.num_ranks_per_model:(rep+1)*self.num_ranks_per_model]
                ...
                for s, (stage, layers_in_stage) in enumerate(self.organized_layers.items()):
                    ...
                    nn_structure[f"sd{sd}"][f"r{rep}"][f"s{s}"] = {
                        "ranks": model_ranks[old_ranks_per_this_stage:old_ranks_per_this_stage+ranks_per_this_stage], "layers": layers_in_stage}
                    ...
                    for sh, rank in zip(range(self.net_dict[layers_in_stage[0]]["num_layer_shards"]), nn_structure[f"sd{sd}"][f"r{rep}"][f"s{s}"]["ranks"]):
                        ...
                        if sd == 0 and rep == 0:
                            # Ranks to synchronize stage with on all subdomains
                            global_ranks = [
                        ...
                        if rep == 0:
                            # Ranks to synchronize stage with on this subdomain
                            local_ranks = [
                                rank + i*self.num_stages for i in range(self.num_replicas_per_subdomain)]
        ...
        while dst:
            ...
            for layer_name in dst:
                ...
                if layer_name not in organized_layers[net[layer_name]['stage']]:
                    organized_layers[net[layer_name]
                                     ['stage']].append(layer_name)
    ...
    def _validate_network(self):
        ...
        for k, v in net.items():
            # This is done to check for cycles in the network graph.
            v["stage"] = 1
