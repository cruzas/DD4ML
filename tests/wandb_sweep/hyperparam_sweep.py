...
from src.utils import distributed_run, dprint, generic_run
...
def main(rank=None, master_addr=None, master_port=None, world_size=None, args=None, wandb_config=None, use_wandb=False):
    ...
    def epoch_end_callback(trainer, save_model=False, save_frequency=5):
        ...
        if save_model and trainer.epoch_num % save_frequency == 0:
            dprint("Saving model...")
            model_path = os.path.join(wandb_config["work_dir"], f"model_epoch_{trainer.epoch_num}.pt")
...
def one_trial_hyperparam_sweep(args):
    ...
    entity = "cruzaslocal"
    project = f"sgd_adam_hyperparameter_sweep"
    num_workers = 1  # or however many local processes you want
...
if __name__ == "__main__":
    ...
    for trial in range(trials):
        dprint(f"Running trial {trial+1}/{trials}")
